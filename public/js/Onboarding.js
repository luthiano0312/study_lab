...
const COLORS = [
    {
        key: "rosa",
        label: "Rosa",
        grad: "linear-gradient(135deg,#be185d 0%,#db2777 40%,#f472b6 100%)",
        light: false,
    },
    {
        key: "roxo",
        label: "Roxo",
        grad: "linear-gradient(135deg,#5b21b6 0%,#7c3aed 40%,#a78bfa 100%)",
        light: false,
    },
    {
        key: "azul",
        label: "Azul",
        grad: "linear-gradient(135deg,#1e40af 0%,#2563eb 40%,#60a5fa 100%)",
        light: false,
    },
    {
        key: "verde",
        label: "Verde",
        grad: "linear-gradient(135deg,#065f46 0%,#059669 40%,#34d399 100%)",
        light: false,
    },
    {
        key: "laranja",
        label: "Laranja",
        grad: "linear-gradient(135deg,#c2410c 0%,#ea580c 40%,#fb923c 100%)",
        light: false,
    },
    {
        key: "preto",
    ...
    },
    {
        key: "vermelho",
        label: "Vermelho",
        grad: "linear-gradient(135deg,#991b1b 0%,#dc2626 40%,#f87171 100%)",
        light: false,
    },
    {
        key: "branco",
        label: "Branco",
        grad: "linear-gradient(135deg,#e5e7eb 0%,#f9fafb 50%,#ffffff 100%)",
        light: true,
    },
    {
        key: "ciano",
    ...
    },
    {
        key: "amarelo",
        label: "Amarelo",
        grad: "linear-gradient(135deg,#92400e 0%,#d97706 40%,#fcd34d 100%)",
        light: false,
    },
    {
        key: "indigo",
    ...
    },
    {
        key: "rose-gold",
        label: "Rose Gold",
        grad: "linear-gradient(135deg,#9f1239 0%,#e11d48 30%,#fb7185 60%,#fda4af 100%)",
        light: false,
    },
];
...
const AVATAR_CATEGORIES = {
    animais: {
        label: "Animais",
        avatars: Array.from({ length: 4 }, (_, i) => ({
            id: i + 1,
...
const state = {
    ...
    avatarType: "preset",
    avatarId: 1,
    avatarUrl: AVATAR_CATEGORIES.animais.avatars[0].url,
    avatarFile: null,
    avatarDataUrl: null,
    userId: null,
    planKey: "free",
    currentAvatarCategory: "animais",
    viewingAllCategories: false,
};
...
async function init() {
    ...
    selectAvatar(AVATAR_CATEGORIES.animais.avatars[0], null);
}
...
function selectPlan(plan) {
    ...
    free.querySelector(".plan-check").classList.toggle(
        "opacity-100",
        plan === "free",
    );
    free.querySelector(".plan-check").classList.toggle(
    ...
    premium.querySelector(".plan-check").style.background =
        plan === "premium" ? "#db2777" : "#e5e7eb";
    premium
        .querySelector(".plan-check")
        .classList.toggle("opacity-100", plan === "premium");
    premium
        .querySelector(".plan-check")
        .classList.toggle("opacity-0", plan !== "premium");
}
...
function buildThemes() {
    ...
    THEMES.forEach((t, idx) => {
        ...
        card.className =
            "relative rounded-xl overflow-hidden border-2 cursor-pointer transition-all duration-200 hover:scale-[1.04] " +
        ...
        const mainLines = [
            ...t.lines.map(
                (c, i) =>
                    `<div class="h-[3px] rounded-sm" style="width:${lw[i % lw.length]}%;background:${c};opacity:.8"></div>`,
            ),
            ...t.lines.map(
                (c, i) =>
                    `<div class="h-[3px] rounded-sm" style="width:${lw[(i + 2) % lw.length]}%;background:${c};opacity:.35"></div>`,
            ),
        ].join("");
        ...
        card.addEventListener("click", () => {
            document.querySelectorAll("#themeGrid > div").forEach((c) => {
                c.classList.remove("border-pink-500");
                c.classList.add("border-gray-200");
                c.querySelector(".check-icon").classList.replace(
                    "opacity-100",
                    "opacity-0",
                );
            });
            ...
            card.querySelector(".check-icon").classList.replace(
                "opacity-0",
                "opacity-100",
            );
...
function buildColors() {
    ...
    COLORS.forEach((c, idx) => {
        ...
        el.className =
            "relative rounded-lg border-2 cursor-pointer overflow-hidden hover:scale-[1.04] transition-all duration-200 " +
            (idx === 0 ? selBorder : "border-transparent");
        el.style.cssText = `background:${c.grad}; aspect-ratio:16/9`;
        el.dataset.key = c.key;
        el.dataset.selBorder = selBorder;

        const labelColor = c.light
            ? "rgba(55,65,81,.75)"
            : "rgba(255,255,255,.85)";
...
function selectColor(c) {
    document.querySelectorAll("#colorGrid > div").forEach((el) => {
        ...
        el.querySelector(".check-overlay").classList.replace(
            "opacity-100",
            "opacity-0",
        );
    });
    ...
    if (cardPreview) cardPreview.style.background = c.grad;
    ...
    setStyle("previewName", "color", isLight ? "#1f2937" : "#ffffff");
    setStyle(
        "previewSchool",
        "color",
        isLight ? "rgba(55,65,81,.55)" : "rgba(255,255,255,.5)",
    );
    setStyle(
        "previewId",
        "color",
        isLight ? "rgba(55,65,81,.35)" : "rgba(255,255,255,.3)",
    );
    setStyle(
        "previewYear",
        "color",
        isLight ? "rgba(55,65,81,.5)" : "rgba(255,255,255,.6)",
    );
    setStyle(
        "previewYear",
        "background",
        isLight ? "rgba(0,0,0,.07)" : "rgba(255,255,255,.15)",
    );

    const mini = document.getElementById("previewPhotoMini");
    if (mini) {
        mini.style.borderColor = isLight
            ? "rgba(0,0,0,.12)"
            : "rgba(255,255,255,.25)";
        mini.style.background = isLight
            ? "rgba(0,0,0,.05)"
            : "rgba(255,255,255,.15)";
    }

    const overlay = document.getElementById("cardPatternOverlay");
    if (overlay) overlay.style.opacity = isLight ? "0.03" : "0.05";
}

function toggleAvatarView() {
    state.viewingAllCategories = !state.viewingAllCategories;
    const btn = document.getElementById("toggleViewBtn");
    const categoryView = document.getElementById("categoryView");
    const allCatsView = document.getElementById("allCategoriesView");

    if (state.viewingAllCategories) {
        categoryView.classList.add("hidden");
        allCatsView.classList.remove("hidden");
        btn.textContent = "Voltar";
    } else {
        categoryView.classList.remove("hidden");
        allCatsView.classList.add("hidden");
        btn.textContent = "Ver Todos";
    }
}

function buildAvatarCategories() {
    const allCatsView = document.getElementById("allCategoriesView");
    if (!allCatsView) return;
    allCatsView.innerHTML = "";

    Object.entries(AVATAR_CATEGORIES).forEach(([key, cat]) => {
        const catCard = document.createElement("div");
        catCard.className =
            "relative rounded-xl overflow-hidden cursor-pointer group";
        catCard.style.cssText = "aspect-ratio: 1/1;";

        catCard.innerHTML = `
            <img src="${cat.avatars[0].url}" alt="${cat.label}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300">
            <div class="absolute inset-0 bg-black/40 group-hover:bg-black/30 transition-colors duration-300 flex items-center justify-center">
                <span class="text-white font-semibold text-[14px]">${cat.label}</span>
            </div>`;

        catCard.addEventListener("click", () => {
            state.currentAvatarCategory = key;
            state.viewingAllCategories = false;
            document.getElementById("categoryView").classList.remove("hidden");
            document
                .getElementById("allCategoriesView")
                .classList.add("hidden");
            document.getElementById("toggleViewBtn").textContent = "Ver Todos";
            buildAvatarGrid(key);
        });

        allCatsView.appendChild(catCard);
    });
}

function buildAvatarGrid(categoryKey) {
    const grid = document.getElementById("avatarGrid");
    const categoryName = document.getElementById("currentCategoryName");
    const category = AVATAR_CATEGORIES[categoryKey];
    if (!grid || !category) return;

    grid.innerHTML = "";
    if (categoryName) categoryName.textContent = category.label;

    category.avatars.forEach((av, i) => {
        const isSelected = i === 0 && state.avatarId === av.id;

        const btn = document.createElement("button");
        btn.className =
            "relative aspect-square rounded-lg border-2 overflow-hidden flex items-center justify-center hover:scale-[1.05] transition-all duration-200 bg-gray-50 " +
        ...
        btn.innerHTML = `
            ...
            <div class="avcheck absolute bottom-1 right-1 w-3 h-3 bg-pink-500 rounded-full flex items-center justify-center ${state.avatarId === av.id ? "opacity-100" : "opacity-0"} transition-opacity duration-200">
...
function selectAvatar(av, btnEl) {
    document.querySelectorAll("#avatarGrid button").forEach((b) => {
        ...
        b.querySelector(".avcheck")?.classList.replace(
            "opacity-100",
            "opacity-0",
        );
    });
...
function bindEvents() {
    ...
    if (nameInput) {
        nameInput.addEventListener("input", () => {
            ...
            state.name = v;
            const previewVal = document.getElementById("namePreviewVal");
            const previewName = document.getElementById("previewName");
            if (previewVal) previewVal.textContent = v ? v.toUpperCase() : "—";
            if (previewName)
                previewName.textContent = v ? v.toUpperCase() : "SEU NOME";
            const nextBtn = document.getElementById("nextStep1");
            if (nextBtn) nextBtn.disabled = v.length < 2;
        });
    }

    const finishBtn = document.getElementById("finishBtn");
    if (finishBtn) finishBtn.addEventListener("click", () => finish(false));
    ...
    document.addEventListener("keydown", (e) => {
        ...
        if (state.currentStep === 2) {
            ...
            const ci = cards.findIndex((c) =>
                c.classList.contains("border-pink-500"),
            );
